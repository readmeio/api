...
sdk.server('https://eu.api.example.com/v14');`),
  ...
  createOperationAccessor(
    ...
    if (shouldAddAltTypedOverloads) {
      ...
      //
      // Thankfully our core fetch work in `api/core` is able to do the proper determination to see
      // if what the user is supplying is `metadata` or `body` content when they supply one or both.
