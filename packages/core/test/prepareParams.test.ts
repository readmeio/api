...
describe('#prepareParams()', function () {
  ...
  describe('content types', function () {
    ...
    describe('image/png', function () {
      it('should support a relative file path payload', async function () {
        ...
        expect(params.body).to.have.string('data:image/png;name=owlbert.png;base64,');
        expect(params.files['owlbert.png']).to.be.an.instanceOf(Buffer);
      });
    ...
    describe('multipart/form-data', function () {
      it('should handle a multipart body when a property is a file path', async function () {
        ...
        const params = await prepareParams(operation, body);
        expect(params.body.documentFile).to.have.string('data:application/json;name=readme.json;base64,');
        expect(params.files['readme.json']).to.be.an.instanceOf(Buffer);
      });
      ...
      it('should handle when the file path is relative', async function () {
        ...
        const params = await prepareParams(operation, body);
        expect(params.body.documentFile).to.have.string('data:image/png;name=owlbert.png;base64,');
        expect(params.files['owlbert.png']).to.be.an.instanceOf(Buffer);
      });
