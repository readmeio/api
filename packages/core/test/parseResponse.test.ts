...
describe('#parseResponse()', function () {
  ...
  describe('#parseResponse', function () {
    ...
    it('should parse non-json response as text', async function () {
      const nonJsonResponseBody = '<xml-stringg />';
      const nonJsonResponse = new Response('<xml-stringg />', {
        headers: {
          'Content-Type': 'application/xml',
        },
      });

      expect(await parseResponse(nonJsonResponse)).to.deep.equal(nonJsonResponseBody);
    });
